...
export default function AdminDashboard() {
  ...
  async function load() {
    try {
      setLoading(true);
      setError("");
      const [statsRes, verificationRes, reportsRes, paymentsRes] =
        await Promise.all([
          getAdminStats(),
          getAdminVerifications(),
          getAdminReports(),
    ...
    if (decision === "rejected") {
      reason =
        window.prompt(
          ...
          "Verification details need clarification",
        ) || "Verification details need clarification";
    }
    try {
      ...
      setError(err.response?.data?.message || "Failed to review verification");
    }
  }
      ...
      <div className="page-container max-w-6xl">
        <div className="mb-8">
          ...
          <p className="section-copy">
            Keep an eye on marketplace activity, verification and payments.
          </p>
        </div>
        {error && (
        ...
        {stats && (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Stat label="Active cars" value={stats.activeCars} />
            ...
            <Stat label="All listings" value={stats.cars} />
            <Stat
        ...
        <section className="mt-8">
          <div className="mb-4">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Paid requests waiting for manual review.
            </p>
          </div>
          {!verifications.length ? (
            <div className="surface p-7 text-sm text-gray-500">
              No paid verification requests are waiting.
            </div>
          ) : (
        ...
        <section className="mt-8">
          ...
          {!reports.length ? (
            ...
            <div className="space-y-3">
              {reports.map((report) => (
                <div key={report._id} className="surface p-5">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      <select
                        value={report.status}
                        onChange={(e) =>
                          updateReport(report._id, e.target.value)
                        }
        ...
        <section className="mt-8">
          <div className="mb-4">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Payment records stored by MyCarsHub.
            </p>
          </div>
          <div className="surface overflow-x-auto">
              ...
              <tbody>
                {payments.map((payment) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-sm capitalize">{payment.type}</td>
                    <td className="p-4 text-sm">
                      ₹{Number(payment.amount).toLocaleString("en-IN")}
                    </td>
                    <td className="p-4 text-sm capitalize">{payment.status}</td>
                    <td className="p-4 text-sm text-gray-500">
